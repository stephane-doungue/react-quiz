...
const ResultOverview: FC<ResultOverviewProps> = ({ result }) => {
  ...
  const calculateStatus =
    (obtainedScore / quizDetails.totalScore) * 100 >= 60 ? 'Réussi' : 'Échec'

  return (
    <ResultOverviewStyle>
      <p>
      Vous avez tenté de poser des questions :{' '}
        <HighlightedText> {totalQuestionAttempted} </HighlightedText>/{' '}
        {quizDetails.totalQuestions}
      </p>
      <p>
      Score sécurisé :<HighlightedText> {obtainedScore} </HighlightedText>/{' '}
